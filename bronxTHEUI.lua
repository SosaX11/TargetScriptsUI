...
getgenv().Bronx = getgenv().Bronx or {}
local Bronx = getgenv().Bronx
Bronx.Directory    = "Bronx_Loader"
Bronx.Folders      = {"/configs"}
Bronx.Flags        = {}
Bronx.ConfigFlags  = {}
Bronx.Connections  = {}
Bronx.Notifications= {Notifs = {}}
Bronx.__index      = Bronx
local Flags          = Bronx.Flags
local ConfigFlags    = Bronx.ConfigFlags
...
for _, path in Bronx.Folders do
    pcall(function() makefolder(Bronx.Directory .. path) end)
end
function Bronx:Tween(Object, Properties, Info)
...
function Bronx:Create(instance, options)
...
function Bronx:Resizify(Parent)
    local UIS = game:GetService("UserInputService")
    local Resizing = Bronx:Create("TextButton", {
    ...
    local grip = Bronx:Create("ImageLabel", {
...
function Bronx:Window(properties)
    ...
    if Bronx.Other then Bronx.Other:Destroy() end
    if Bronx.ToggleGui then Bronx.ToggleGui:Destroy() end
    Bronx.Gui = Bronx:Create("ScreenGui", { Parent = CoreGui, Name = "ORGGUI", Enabled = true, IgnoreGuiInset = true, ZIndexBehavior = Enum.ZIndexBehavior.Sibling })
    Bronx.Other = Bronx:Create("ScreenGui", { Parent = CoreGui, Name = "ORGOther", Enabled = false, IgnoreGuiInset = true })
    local Items = Cfg.Items
    local uiVisible = true
    Items.Wrapper = Bronx:Create("Frame", {
        Parent = Bronx.Gui, Position = dim2(0.5, -Cfg.Size.X.Offset / 2, 0.5, -Cfg.Size.Y.Offset / 2),
        Size = Cfg.Size, BackgroundTransparency = 1, BorderSizePixel = 0
    })
    Items.Window = Bronx:Create("Frame", {
    ...
    Bronx:Themify(Items.Window, "background", "BackgroundColor3")
    Bronx:Create("UICorner", { Parent = Items.Window, CornerRadius = dim(0, 14) })
    Bronx:Themify(Bronx:Create("UIStroke", { Parent = Items.Window, Color = themes.preset.outline, Thickness = 1 }), "outline", "Color")
    Items.Sidebar = Bronx:Create("Frame", {
    ...
    Bronx:Themify(Items.Sidebar, "section", "BackgroundColor3")
    Bronx:Create("UICorner", { Parent = Items.Sidebar, CornerRadius = dim(0, 12) })
    Bronx:Themify(Bronx:Create("UIStroke", { Parent = Items.Sidebar, Color = themes.preset.outline, Thickness = 1 }), "outline", "Color")
    Items.TabHolder = Bronx:Create("ScrollingFrame", {
    ...
    Bronx:Create("UIPadding", { Parent = Items.TabHolder, PaddingTop = dim(0, 14), PaddingBottom = dim(0, 14) })
    Items.Header = Bronx:Create("Frame", {
    ...
    Bronx:Themify(Items.AvatarFrame, "element", "BackgroundColor3")
    Bronx:Create("UICorner", { Parent = Items.AvatarFrame, CornerRadius = dim(1, 0) })
    Bronx:Themify(Bronx:Create("UIStroke", { Parent = Items.AvatarFrame, Color = themes.preset.outline, Thickness = 1 }), "outline", "Color")
    Items.Avatar = Bronx:Create("ImageLabel", {
    ...
    Bronx:Create("UICorner", { Parent = Items.Avatar, CornerRadius = dim(1, 0) })
    Items.Username = Bronx:Create("TextLabel", {
    ...
    Bronx:Themify(Items.Username, "text", "TextColor3")
    Items.SettingsBtn = Bronx:Create("ImageButton", {
    ...
    Bronx:Themify(Items.SettingsBtn, "subtext", "ImageColor3")
    ...
    Items.PageHolder = Bronx:Create("Frame", {
    ...
    if InputService.TouchEnabled then
        ...
        Bronx:Create("UIStroke", { Parent = ToggleButton, Color = themes.preset.accent, Thickness = 1.5 })
    ...
    return setmetatable(Cfg, Bronx)
end
function Bronx:Tab(properties)
    local Cfg = {
    ...
    if not Cfg.Hidden then
        Items.Button = Bronx:Create("TextButton", {
        ...
        Bronx:Themify(Items.Button, "tab_active", "BackgroundColor3")
        Bronx:Create("UICorner", { Parent = Items.Button, CornerRadius = dim(0.5, 0) })
        Items.IconImg = Bronx:Create("ImageLabel", {
        ...
        Bronx:Themify(Items.IconImg, "subtext", "ImageColor3")
    end
    Items.Pages = Bronx:Create("CanvasGroup", { Parent = Bronx.Other, Size = dim2(1, 0, 1, 0), BackgroundTransparency = 1, Visible = false, GroupTransparency = 1 })
    Bronx:Create("UIListLayout", { Parent = Items.Pages, FillDirection = Enum.FillDirection.Horizontal, Padding = dim(0, 14) })
    Bronx:Create("UIPadding", { Parent = Items.Pages, PaddingTop = dim(0, 2), PaddingBottom = dim(0, 14), PaddingRight = dim(0, 2), PaddingLeft = dim(0, 2) })
    Items.Left = Bronx:Create("ScrollingFrame", {
    ...
    Bronx:Create("UIListLayout", { Parent = Items.Left, Padding = dim(0, 14) })
    Bronx:Create("UIPadding", { Parent = Items.Left, PaddingBottom = dim(0, 10) })
    Items.Right = Bronx:Create("ScrollingFrame", {
        Parent = Items.Pages, Size = dim2(0.5, -7, 1, 0), BackgroundTransparency = 1,
        ScrollBarThickness = 0, CanvasSize = dim2(0, 0, 0, 0), AutomaticCanvasSize = Enum.AutomaticSize.Y
    })
    Bronx:Create("UIListLayout", { Parent = Items.Right, Padding = dim(0, 14) })
    Bronx:Create("UIPadding", { Parent = Items.Right, PaddingBottom = dim(0, 10) })
    function Cfg.OpenTab()
        ...
        if oldTab and oldTab.Button then
            Bronx:Tween(oldTab.Button, {BackgroundTransparency = 1}, buttonTween)
            Bronx:Tween(oldTab.IconImg, {ImageColor3 = themes.preset.subtext}, buttonTween)
        end
        if Items.Button then
            Bronx:Tween(Items.Button, {BackgroundTransparency = 0}, buttonTween)
            Bronx:Tween(Items.IconImg, {ImageColor3 = rgb(255,255,255)}, buttonTween)
        end
        task.spawn(function()
            if oldTab then
                Bronx:Tween(oldTab.Pages, {GroupTransparency = 1, Size = dim2(0.95, 0, 0.95, 0)}, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out))
                task.wait(0.2)
                oldTab.Pages.Visible = false
                oldTab.Pages.Parent = Bronx.Other
            end
            ...
            Bronx:Tween(Items.Pages, {GroupTransparency = 0, Size = dim2(1, 0, 1, 0)}, TweenInfo.new(0.4, Enum.EasingStyle.Back, Enum.EasingDirection.Out))
            task.wait(0.35)
...
function Bronx:Section(properties)
    local Cfg = {
    ...
    local Items = Cfg.Items
    Items.Section = Bronx:Create("Frame", {
    ...
    Bronx:Themify(Items.Section, "section", "BackgroundColor3")
    Bronx:Create("UICorner", { Parent = Items.Section, CornerRadius = dim(0, 12) })
    Bronx:Themify(Bronx:Create("UIStroke", { Parent = Items.Section, Color = themes.preset.outline, Thickness = 1 }), "outline", "Color")
    Items.Header = Bronx:Create("Frame", { Parent = Items.Section, Size = dim2(1, 0, 0, 38), BackgroundTransparency = 1 })
    Items.Dot = Bronx:Create("Frame", {
    ...
    Bronx:Themify(Items.Dot, "accent", "BackgroundColor3")
    Bronx:Create("UICorner", { Parent = Items.Dot, CornerRadius = dim(1, 0) })
    Items.Title = Bronx:Create("TextLabel", {
    ...
    Bronx:Themify(Items.Title, "text", "TextColor3")
    Items.Container = Bronx:Create("Frame", {
        Parent = Items.Section, Position = dim2(0, 0, 0, 38), Size = dim2(1, 0, 0, 0),
        AutomaticSize = Enum.AutomaticSize.Y, BackgroundTransparency = 1
    })
    Bronx:Create("UIListLayout", { Parent = Items.Container, Padding = dim(0, 8), SortOrder = Enum.SortOrder.LayoutOrder })
    Bronx:Create("UIPadding", { Parent = Items.Container, PaddingBottom = dim(0, 12), PaddingLeft = dim(0, 14), PaddingRight = dim(0, 14) })
    return setmetatable(Cfg, Bronx)
end
function Bronx:ScriptCard(properties)
    local Cfg = {
    ...
    Bronx:Themify(Items.Container, "element", "BackgroundColor3")
    Bronx:Create("UICorner", { Parent = Items.Container, CornerRadius = dim(0, 8) })
    Bronx:Themify(Bronx:Create("UIStroke", { Parent = Items.Container, Color = themes.preset.outline, Thickness = 1 }), "outline", "Color")
    Items.IconFrame = Bronx:Create("Frame", {
    ...
    Bronx:Themify(Items.IconFrame, "background", "BackgroundColor3")
    Bronx:Create("UICorner", { Parent = Items.IconFrame, CornerRadius = dim(0, 8) })
    Items.Icon = Bronx:Create("ImageLabel", {
    ...
    Bronx:Themify(Items.Icon, "accent", "ImageColor3")
    Items.Title = Bronx:Create("TextLabel", {
    ...
    Bronx:Themify(Items.Title, "text", "TextColor3")
    Items.Desc = Bronx:Create("TextLabel", {
    ...
    Bronx:Themify(Items.Desc, "subtext", "TextColor3")
    Items.LoadBtn = Bronx:Create("TextButton", {
    ...
    Bronx:Themify(Items.LoadBtn, "accent", "BackgroundColor3")
    Bronx:Create("UICorner", { Parent = Items.LoadBtn, CornerRadius = dim(0, 6) })
    Items.LoadBtn.MouseButton1Click:Connect(function()
        Bronx:Tween(Items.LoadBtn, {BackgroundTransparency = 0.4, Size = dim2(0, 56, 0, 28)}, TweenInfo.new(0.1))
        task.wait(0.1)
        Bronx:Tween(Items.LoadBtn, {BackgroundTransparency = 0, Size = dim2(0, 60, 0, 30)}, TweenInfo.new(0.2))
        Cfg.Callback()
    end)
    return setmetatable(Cfg, Bronx)
end
function Bronx:Toggle(properties)
    local Cfg = {
    ...
    Bronx:Themify(Items.Button, "element", "BackgroundColor3")
    Bronx:Create("UICorner", { Parent = Items.Button, CornerRadius = dim(0, 8) })
    Bronx:Themify(Bronx:Create("UIStroke", { Parent = Items.Button, Color = themes.preset.outline, Thickness = 1 }), "outline", "Color")
    Items.SwitchBG = Bronx:Create("Frame", {
    ...
    Bronx:Themify(Items.SwitchBG, "background", "BackgroundColor3")
    Bronx:Create("UICorner", { Parent = Items.SwitchBG, CornerRadius = dim(1, 0) })
    Bronx:Themify(Bronx:Create("UIStroke", { Parent = Items.SwitchBG, Color = themes.preset.outline, Thickness = 1 }), "outline", "Color")
    Items.SwitchKnob = Bronx:Create("Frame", {
    ...
    Bronx:Themify(Items.SwitchKnob, "subtext", "BackgroundColor3")
    Bronx:Create("UICorner", { Parent = Items.SwitchKnob, CornerRadius = dim(1, 0) })
    Items.Title = Bronx:Create("TextLabel", {
    ...
    Bronx:Themify(Items.Title, "subtext", "TextColor3")
    local State = false
    function Cfg.set(bool)
        State = bool
        Bronx:Tween(Items.SwitchBG, {BackgroundColor3 = State and themes.preset.accent or themes.preset.background}, TweenInfo.new(0.25, Enum.EasingStyle.Quart, Enum.EasingDirection.Out))
        Bronx:Tween(Items.SwitchKnob, {Position = State and dim2(0, 19, 0.5, 0) or dim2(0, 3, 0.5, 0), BackgroundColor3 = State and rgb(255,255,255) or themes.preset.subtext}, TweenInfo.new(0.25, Enum.EasingStyle.Back, Enum.EasingDirection.Out))
        Bronx:Tween(Items.Title, {TextColor3 = State and themes.preset.text or themes.preset.subtext}, TweenInfo.new(0.2))
...
function Bronx:Button(properties)
    local Cfg = {
    ...
    Bronx:Themify(Items.Button, "element", "BackgroundColor3")
    Bronx:Themify(Items.Button, "text", "TextColor3")
    Bronx:Create("UICorner", { Parent = Items.Button, CornerRadius = dim(0, 8) })
    Bronx:Themify(Bronx:Create("UIStroke", { Parent = Items.Button, Color = themes.preset.outline, Thickness = 1 }), "outline", "Color")
    Items.Button.MouseButton1Click:Connect(function()
        Bronx:Tween(Items.Button, {Size = dim2(0.98, 0, 0, 34), BackgroundColor3 = themes.preset.accent}, TweenInfo.new(0.1))
        task.wait(0.1)
        Bronx:Tween(Items.Button, {Size = dim2(1, 0, 0, 36), BackgroundColor3 = themes.preset.element}, TweenInfo.new(0.3, Enum.EasingStyle.Back, Enum.EasingDirection.Out))
        Cfg.Callback()
    end)
    return setmetatable(Cfg, Bronx)
end
function Bronx:Slider(properties)
    local Cfg = {
    ...
    local Items = Cfg.Items
    Items.ContainerBox = Bronx:Create("Frame", {
        Parent = self.Items.Container, Size = dim2(1, 0, 0, 52),
        BackgroundColor3 = themes.preset.element
    })
    Bronx:Themify(Items.ContainerBox, "element", "BackgroundColor3")
    Bronx:Create("UICorner", { Parent = Items.ContainerBox, CornerRadius = dim(0, 8) })
    Bronx:Themify(Bronx:Create("UIStroke", { Parent = Items.ContainerBox, Color = themes.preset.outline, Thickness = 1 }), "outline", "Color")
    Items.Title = Bronx:Create("TextLabel", { Parent = Items.ContainerBox, Position = dim2(0, 14, 0, 6), Size = dim2(1, -28, 0, 20), BackgroundTransparency = 1, Text = Cfg.Name, TextColor3 = themes.preset.text, TextSize = 13, FontFace = Font.new("rbxassetid://12187365364", Enum.FontWeight.Medium), TextXAlignment = Enum.TextXAlignment.Left })
    Bronx:Themify(Items.Title, "text", "TextColor3")
    Items.Val = Bronx:Create("TextLabel", { Parent = Items.ContainerBox, Position = dim2(0, 14, 0, 6), Size = dim2(1, -28, 0, 20), BackgroundTransparency = 1, Text = tostring(Cfg.Default)..Cfg.Suffix, TextColor3 = themes.preset.accent, TextSize = 13, FontFace = Font.new("rbxassetid://12187365364", Enum.FontWeight.Bold), TextXAlignment = Enum.TextXAlignment.Right })
    Bronx:Themify(Items.Val, "accent", "TextColor3")
    Items.Track = Bronx:Create("TextButton", { Parent = Items.ContainerBox, Position = dim2(0, 14, 0, 34), Size = dim2(1, -28, 0, 8), BackgroundColor3 = themes.preset.background, Text = "", AutoButtonColor = false })
    Bronx:Themify(Items.Track, "background", "BackgroundColor3")
    Bronx:Create("UICorner", { Parent = Items.Track, CornerRadius = dim(1, 0) })
    Items.Fill = Bronx:Create("Frame", { Parent = Items.Track, Size = dim2(0, 0, 1, 0), BackgroundColor3 = themes.preset.accent })
    Bronx:Themify(Items.Fill, "accent", "BackgroundColor3")
    Bronx:Create("UICorner", { Parent = Items.Fill, CornerRadius = dim(1, 0) })
    Items.Knob = Bronx:Create("Frame", {Parent = Items.Fill, AnchorPoint = vec2(1, 0.5), Position = dim2(1, 4, 0.5, 0), Size = dim2(0, 14, 0, 14), BackgroundColor3 = rgb(255,255,255)})
    Bronx:Create("UICorner", { Parent = Items.Knob, CornerRadius = dim(1, 0) })
    ...
    function Cfg.set(val)
        ...
        Bronx:Tween(Items.Fill, {Size = dim2((Value - Cfg.Min) / (Cfg.Max - Cfg.Min), 0, 1, 0)}, TweenInfo.new(0.15))
...
function Bronx:Textbox(properties)
    local Cfg = {
    ...
    Bronx:Create("UICorner", { Parent = Items.ContainerBox, CornerRadius = dim(0, 8) })
    Bronx:Themify(Bronx:Create("UIStroke", { Parent = Items.ContainerBox, Color = themes.preset.outline, Thickness = 1 }), "outline", "Color")
    Items.Input = Bronx:Create("TextBox", {
    ...
    Bronx:Themify(Items.Input, "text", "TextColor3")
    function Cfg.set(val)
...
function Bronx:Dropdown(properties)
    local Cfg = {
    ...
    Bronx:Create("UICorner", { Parent = Items.ContainerBox, CornerRadius = dim(0, 8) })
    Bronx:Themify(Bronx:Create("UIStroke", { Parent = Items.ContainerBox, Color = themes.preset.outline, Thickness = 1 }), "outline", "Color")
    Items.Main = Bronx:Create("TextButton", {
        Parent = Items.ContainerBox, Size = dim2(1, 0, 1, 0), BackgroundTransparency = 1, Text = "", AutoButtonColor = false
    })
    Items.Title = Bronx:Create("TextLabel", { Parent = Items.Main, Position = dim2(0, 14, 0, 0), Size = dim2(0, 100, 1, 0), BackgroundTransparency = 1, Text = Cfg.Name, TextColor3 = themes.preset.subtext, TextSize = 13, FontFace = Font.new("rbxassetid://12187365364", Enum.FontWeight.Medium), TextXAlignment = Enum.TextXAlignment.Left })
    Bronx:Themify(Items.Title, "subtext", "TextColor3")
    Items.SelectedText = Bronx:Create("TextLabel", { Parent = Items.Main, Position = dim2(0, 114, 0, 0), Size = dim2(1, -142, 1, 0), BackgroundTransparency = 1, Text = "...", TextColor3 = themes.preset.text, TextSize = 13, FontFace = Font.new("rbxassetid://12187365364", Enum.FontWeight.Medium), TextXAlignment = Enum.TextXAlignment.Right })
    Bronx:Themify(Items.SelectedText, "text", "TextColor3")
    Items.Icon = Bronx:Create("ImageLabel", { Parent = Items.Main, Position = dim2(1, -20, 0.5, 0), AnchorPoint = vec2(0, 0.5), Size = dim2(0, 12, 0, 12), BackgroundTransparency = 1, Image = "rbxassetid://10723415903", ImageColor3 = themes.preset.subtext, Rotation = -90 })
    Items.DropFrame = Bronx:Create("CanvasGroup", {
        Parent = Bronx.Gui, Size = dim2(1, 0, 0, 0), Position = dim2(0, 0, 0, 0),
        BackgroundColor3 = themes.preset.element, Visible = false, ZIndex = 200, GroupTransparency = 0
    })
    Bronx:Themify(Items.DropFrame, "element", "BackgroundColor3")
    Bronx:Create("UICorner", { Parent = Items.DropFrame, CornerRadius = dim(0, 8) })
    Bronx:Themify(Bronx:Create("UIStroke", { Parent = Items.DropFrame, Color = themes.preset.outline, Thickness = 1 }), "outline", "Color")
    Items.Scroll = Bronx:Create("ScrollingFrame", {
    ...
    Bronx:Create("UIListLayout", { Parent = Items.Scroll, SortOrder = Enum.SortOrder.LayoutOrder })
    local Open = false
    ...
    local function ToggleDropdown()
        ...
        if Open then
            ...
            Bronx:Tween(Items.Icon, {Rotation = 90}, TweenInfo.new(0.3))
            local tw = Bronx:Tween(Items.DropFrame, {Size = dim2(0, Items.Main.AbsoluteSize.X, 0, targetHeight)}, TweenInfo.new(0.4, Enum.EasingStyle.Back, Enum.EasingDirection.Out))
            tw.Completed:Wait()
        else
            Bronx:Tween(Items.Icon, {Rotation = -90}, TweenInfo.new(0.3))
            local tw = Bronx:Tween(Items.DropFrame, {Size = dim2(0, Items.Main.AbsoluteSize.X, 0, 0)}, TweenInfo.new(0.3, Enum.EasingStyle.Quint, Enum.EasingDirection.Out))
            tw.Completed:Wait()
    ...
    function Cfg.RefreshOptions(newList)
        ...
        for _, opt in ipairs(Cfg.Options) do
            local btn = Bronx:Create("TextButton", {
            ...
            Bronx:Themify(btn, "text", "TextColor3")
...
function Bronx:Label(properties)
    local Cfg = {
...
function Bronx:Colorpicker(properties)
    local Cfg = {
    ...
    Bronx:Create("UICorner", {Parent = btn, CornerRadius = dim(0, 6)})
    Bronx:Create("UIStroke", {Parent = btn, Color = rgb(0,0,0), Thickness = 1, Transparency = 0.5})
    local h, s, v = Color3.toHSV(Cfg.Color)
    Items.DropFrame = Bronx:Create("Frame", { Parent = Bronx.Gui, Size = dim2(0, 160, 0, 0), BackgroundColor3 = themes.preset.element, Visible = false, ZIndex = 200, ClipsDescendants = true })
    Bronx:Themify(Items.DropFrame, "element", "BackgroundColor3")
    Bronx:Create("UICorner", { Parent = Items.DropFrame, CornerRadius = dim(0, 8) })
    Bronx:Themify(Bronx:Create("UIStroke", { Parent = Items.DropFrame, Color = themes.preset.outline, Thickness = 1 }), "outline", "Color")
    Items.SVMap = Bronx:Create("TextButton", { Parent = Items.DropFrame, Position = dim2(0, 8, 0, 8), Size = dim2(1, -16, 1, -38), AutoButtonColor = false, Text = "", BackgroundColor3 = Color3.fromHSV(h, 1, 1), ZIndex = 201 })
    Bronx:Create("UICorner", { Parent = Items.SVMap, CornerRadius = dim(0, 6) })
    Items.SVImage = Bronx:Create("ImageLabel", { Parent = Items.SVMap, Size = dim2(1, 0, 1, 0), Image = "rbxassetid://4155801252", BackgroundTransparency = 1, BorderSizePixel = 0, ZIndex = 202 })
    Bronx:Create("UICorner", { Parent = Items.SVImage, CornerRadius = dim(0, 6) })
    Items.SVKnob = Bronx:Create("Frame", { Parent = Items.SVMap, AnchorPoint = vec2(0.5, 0.5), Size = dim2(0, 8, 0, 8), BackgroundColor3 = rgb(255,255,255), ZIndex = 203 })
    Bronx:Create("UICorner", { Parent = Items.SVKnob, CornerRadius = dim(1, 0) })
    Bronx:Create("UIStroke", { Parent = Items.SVKnob, Color = rgb(0,0,0) })
    Items.HueBar = Bronx:Create("TextButton", { Parent = Items.DropFrame, Position = dim2(0, 8, 1, -22), Size = dim2(1, -16, 0, 14), AutoButtonColor = false, Text = "", BorderSizePixel = 0, BackgroundColor3 = rgb(255, 255, 255), ZIndex = 201 })
    Bronx:Create("UICorner", { Parent = Items.HueBar, CornerRadius = dim(0, 6) })
    Bronx:Create("UIGradient", { Parent = Items.HueBar, Color = ColorSequence.new({ColorSequenceKeypoint.new(0, rgb(255,0,0)), ColorSequenceKeypoint.new(0.167, rgb(255,0,255)), ColorSequenceKeypoint.new(0.333, rgb(0,0,255)), ColorSequenceKeypoint.new(0.5, rgb(0,255,255)), ColorSequenceKeypoint.new(0.667, rgb(0,255,0)), ColorSequenceKeypoint.new(0.833, rgb(255,255,0)), ColorSequenceKeypoint.new(1, rgb(255,0,0))}) })
    Items.HueKnob = Bronx:Create("Frame", { Parent = Items.HueBar, AnchorPoint = vec2(0.5, 0.5), Size = dim2(0, 6, 1, 4), BackgroundColor3 = rgb(255,255,255), ZIndex = 203 })
    Bronx:Create("UIStroke", { Parent = Items.HueKnob, Color = rgb(0,0,0) })
    Bronx:Create("UICorner", { Parent = Items.HueKnob, CornerRadius = dim(0, 3) })
    local Open = false
    local isTweening = false
    local function Toggle()
        ...
        if Open then
            Items.DropFrame.Visible = true
            local tw = Bronx:Tween(Items.DropFrame, {Size = dim2(0, 160, 0, 150)}, TweenInfo.new(0.4, Enum.EasingStyle.Back, Enum.EasingDirection.Out))
            tw.Completed:Wait()
        else
            local tw = Bronx:Tween(Items.DropFrame, {Size = dim2(0, 160, 0, 0)}, TweenInfo.new(0.3, Enum.EasingStyle.Quint, Enum.EasingDirection.Out))
            tw.Completed:Wait()
            Items.DropFrame.Visible = false
        end
        isTweening = false
...
function Bronx:Keybind(properties)
    local Cfg = {
    ...
    local attachParent = self.Items.ContainerBox or self.Items.Button or self.Items.Container
    local KeyBtn = Bronx:Create("TextButton", { Parent = attachParent, AnchorPoint = vec2(1, 0.5), Position = dim2(1, -12, 0.5, 0), Size = dim2(0, 44, 0, 22), BackgroundColor3 = themes.preset.background, TextColor3 = themes.preset.text, Text = Keys[Cfg.Default] or "None", TextSize = 12, FontFace = Font.new("rbxassetid://12187365364", Enum.FontWeight.SemiBold), })
    Bronx:Themify(KeyBtn, "background", "BackgroundColor3")
    Bronx:Themify(KeyBtn, "text", "TextColor3")
    Bronx:Create("UICorner", {Parent = KeyBtn, CornerRadius = dim(0, 6)})
    Bronx:Themify(Bronx:Create("UIStroke", { Parent = KeyBtn, Color = themes.preset.outline, Thickness = 1 }), "outline", "Color")
    local binding = false
...
function Notifications:RefreshNotifications()
    ...
    for _, v in ipairs(Notifications.Notifs) do
        ...
        Bronx:Tween(v, {Position = dim_offset(20, offset)}, TweenInfo.new(0.4, Enum.EasingStyle.Quint, Enum.EasingDirection.Out))
        offset += (ySize + 10)
    end
end
function Notifications:Create(properties)
    ...
    local Items = Cfg.Items
    Items.Outline = Bronx:Create("Frame", { Parent = Bronx.Gui; Position = dim_offset(-500, 50); Size = dim2(0, 300, 0, 0); AutomaticSize = Enum.AutomaticSize.Y; BackgroundColor3 = themes.preset.element; BorderSizePixel = 0; ZIndex = 300, ClipsDescendants = true })
    Bronx:Themify(Items.Outline, "element", "BackgroundColor3")
    Bronx:Create("UICorner", { Parent = Items.Outline, CornerRadius = dim(0, 8) })
    Bronx:Themify(Bronx:Create("UIStroke", { Parent = Items.Outline, Color = themes.preset.outline, Thickness = 1 }), "outline", "Color")
    Items.Name = Bronx:Create("TextLabel", {
    ...
    Bronx:Themify(Items.Name, "text", "TextColor3")
    Bronx:Create("UIPadding", { Parent = Items.Name; PaddingTop = dim(0, 12); PaddingBottom = dim(0, 12); PaddingRight = dim(0, 14); PaddingLeft = dim(0, 14); })
    Items.TimeBar = Bronx:Create("Frame", { Parent = Items.Outline, AnchorPoint = vec2(0, 1), Position = dim2(0, 0, 1, 0), Size = dim2(1, 0, 0, 3), BackgroundColor3 = themes.preset.accent, BorderSizePixel = 0, ZIndex = 303 })
    Bronx:Themify(Items.TimeBar, "accent", "BackgroundColor3")
    ...
    task.spawn(function()
        ...
        Bronx:Tween(Items.TimeBar, {Size = dim2(0, 0, 0, 3)}, TweenInfo.new(Cfg.Lifetime, Enum.EasingStyle.Linear))
        task.wait(Cfg.Lifetime)
        Bronx:Tween(Items.Outline, {Position = dim_offset(-Items.Outline.AbsoluteSize.X - 50, Items.Outline.Position.Y.Offset)}, TweenInfo.new(0.4, Enum.EasingStyle.Quint, Enum.EasingDirection.In))
        task.wait(0.4)
...
local ConfigHolder
function Bronx:UpdateConfigList()
    if not ConfigHolder then return end
    local List = {}
    for _, file in listfiles(Bronx.Directory .. "/configs") do
        local Name = file:gsub(Bronx.Directory .. "/configs\\", ""):gsub(".cfg", ""):gsub(Bronx.Directory .. "\\configs\\", "")
        List[#List + 1] = Name
    end
    ConfigHolder.RefreshOptions(List)
end
function Bronx:Configs(window)
    local Text
    ...
    Section:Button({
        ...
        Callback = function()
            if Flags["config_Name_text"] == "" then return end
            Bronx:LoadConfig(readfile(Bronx.Directory .. "/configs/" .. Flags["config_Name_text"] .. ".cfg"))
            Bronx:UpdateConfigList()
    ...
    Section:Button({
        ...
        Callback = function()
            if Flags["config_Name_text"] == "" then return end
            delfile(Bronx.Directory .. "/configs/" .. Flags["config_Name_text"] .. ".cfg")
            Bronx:UpdateConfigList()
    ...
    SectionRight:Label({Name = "Accent Color"}):Colorpicker({ Callback = function(color3) Bronx:RefreshTheme("accent", color3) end, Color = themes.preset.accent })
    SectionRight:Label({Name = "Background Color"}):Colorpicker({ Callback = function(color3) Bronx:RefreshTheme("background", color3) end, Color = themes.preset.background })
    SectionRight:Label({Name = "Section Color"}):Colorpicker({ Callback = function(color3) Bronx:RefreshTheme("section", color3) end, Color = themes.preset.section })
    SectionRight:Label({Name = "Element Color"}):Colorpicker({ Callback = function(color3) Bronx:RefreshTheme("element", color3) end, Color = themes.preset.element })
    SectionRight:Label({Name = "Text Color"}):Colorpicker({ Callback = function(color3) Bronx:RefreshTheme("text", color3) end, Color = themes.preset.text })
